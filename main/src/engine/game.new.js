...
function starChunk(game, center, size, count) {
  for (let i = 0; i < count; i++) {
    ...
    game.world.chunkList.createStaticItem(
      mt,
...
function mission1(game) {
  let baseSpawner = game.addTimer(15, () => {
    new Collectable(
      game,
      new Vector3d(rand(100) - 50, rand(100) - 50, rand(100) - 50),
  ...
  for (let i = 0; i < 1; i++) {
    let big = game.world.createSolid(
      randVector(new Vector3d(500, 0, 0), 500),
      1,
      { r: Math.random(), g: Math.random(), b: 0.5 },
      "bigModel"
    );
...
function mission2(game) {
  ...
  for (let i = 0; i < 1; i++) {
    let big = game.world.createSolid(
      randVector(new Vector3d(500, 500, 0), 500),
      1,
      { r: Math.random(), g: Math.random(), b: 0.5 },
      "bigModel"
    );
...
function mission4(game) {
  ...
  let point1 = basics.makeCollactable(
    game.world,
    enBasePos,
    10,
    game.world.boxModelList
  );

  let p2 = game.world.createMagic(new Vector3d(0, 0, 0), 100, false);

  let brp = basics.makeBreakableStrong(
    game.world,
    new Vector3d(50, 0, 0),
    1,
...
function mission3(game) {
    ...
    () => {
      for (let i = 0; i < 12; i++) {
        let en = new Enemy(
          game.gl,
          game,
          randVector(new Vector3d(0, -500, -500), 500),
          new Vector3d(0, 0, 0),
          game.world.shipLists[rand(3)]
        );
      }
    }
  );
  ...
  rou.forEach((it) => {
    ...
    let brp = basics.makeBreakableExplosive(
      game.world,
      it,
      0.1,
      game.world.meteModelList,
      10,
      30,
      (bullet) => {
        brp.deleteSelf();
