...
class Enemy extends GameObject{
  ...
  constructor(gl: WebGLRenderingContext, game: Game, startPoint: Vector3d, speedVector: Vector3d, modelList: RenderableModelList, extLogic: (enemy: Enemy) => void){
    ...
    msg.onProcess = ()=>{
      msg.vector = this.pos;
      msg.text = this.msgPref+Math.round(game.player.camera.getPosVector().subVector(msg.vector).abs()*10)/10+ 'km';
    }
 ...
 // }
  ...
  render_( deltaTime: number){
    if (this.extLogic){
      ...
      if (this.atackObject && this.atackObject.hitPosition){
        this.logic(this.atackObject.hitPosition, new Vector3d (0,0,0), deltaTime);
      } else {
